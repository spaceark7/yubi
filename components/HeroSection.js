...
const HeroSection = ({ customers, data }) => {
  return (
    <div className='z-0 overflow-x-hidden'>
      <div className='relative z-0 min-h-screen overflow-x-hidden pt-28 sm:pt-24 md:pt-32'>
        <Layout>
          <div className='h-[75vh] md:h-[70vh] lg:px-10'>
            <div
              className={`${
                data?.is_dark ? 'text-white' : 'text-gray-800'
              } my-auto h-full md:pt-20`}
            >
              <h1 className='max-w-lg text-4xl font-bold drop-shadow-md sm:text-5xl md:text-6xl lg:max-w-md lg:leading-[64px] xl:max-w-xl'>
                {data?.title}
              </h1>
              <p className='mt-6 text-xl opacity-95 sm:max-w-md lg:max-w-xl'>
                {data?.description}
              </p>
              {data?.disable_slider ? null : (
                <div
                  className={`${
                    !data.is_dark
                      ? 'btn mt-12 bg-primaryBrand hover:bg-primaryBrand/80 hover:text-opacity-100 hover:shadow-md active:scale-95 active:bg-primaryBrand xl:text-xl'
                      : 'btn mt-12 bg-tertiaryBrand hover:text-opacity-100 hover:shadow-md active:scale-95 active:bg-tertiaryBrand xl:text-xl'
                  } lg:px-6`}
                >
              ...
              <div className='flex flex-col lg:flex-row '>
                ...
                {data?.is_contact && (
                  <div
                   ...
                   lg:mt-12 lg:ml-4 lg:px-6 xl:text-xl`}
                  >
                    Send Email
                  </div>
                )}
